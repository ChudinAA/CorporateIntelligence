...
class OpenAIService:
    ...
    def generate_response(self, prompt, max_tokens=1024, temperature=0.7, system_prompt=None):
        ...
        try:
            ...
            max_tokens = min(max(1, int(max_tokens)), 4000)  # Keep tokens in reasonable range
            temperature = min(max(0.0, float(temperature)), 1.0)  # Keep temperature in valid range
            ...
            if system_prompt:
                ...
                messages.append({
                    "role": "system", 
                    "content": "You are a helpful, professional AI assistant for a company knowledge system."
                })
    ...
    def rag_prompt_with_context(self, user_query, context, chat_history=None):
        ...
        try:
            ...
            if not user_query or not isinstance(user_query, str):
                user_query = str(user_query) if user_query else "Can you help me find information?"
            
            if not context or not isinstance(context, str):
                context = str(context) if context else "No relevant documents found."
            ...
            try:
                ...
                logger.error(f"RAG API call to OpenAI failed: {str(api_error)}")
                raise api_error
            
        except Exception as e:
            logger.error(f"Error processing RAG query with OpenAI: {str(e)}", exc_info=True)
    ...
    def summarize_chat(self, messages):
        ...
        try:
            ...
            if not messages or not isinstance(messages, list) or len(messages) < 2:
                logger.warning(f"Invalid messages format for summarization: {type(messages)}")
            ...
            api_messages = [
                {
                    "role": "system", 
                    "content": """You are a highly efficient summarization specialist.
            ...
            for msg in messages:
                ...
                chat_content.append(f"{'User' if is_user else 'Assistant'}: {content.strip()}")
                msg_count += 1
            ...
            api_messages.append({
                "role": "user",
                "content": "Based on the conversation above, provide a brief, informative summary in 2-3 sentences."
            })
            ...
            try:
                ...
                logger.error(f"Summary API call to OpenAI failed: {str(api_error)}")
                raise api_error
            
        except Exception as e:
            logger.error(f"Error generating chat summary with OpenAI: {str(e)}", exc_info=True)
